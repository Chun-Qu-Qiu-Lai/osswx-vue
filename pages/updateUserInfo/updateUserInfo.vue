<template>
	<view>
		...
		<!-- <input placeholder="请输入昵称" v-model="nickName" id="nickname-input" type="nickname" @blur="getNickName" /> -->
		<button class="cu-btn bg-blue lg shadow"
			style="width: 50%;margin:30% auto;align-items: center;text-align: center;display: flex;"
			@click="updateUserInfo()">确认</button>
	...
	export default {
		...
		methods: {
			...
			updateUserInfo() {
				...
				uni.request({
					url: requestUrl.prod.BASE_URL + "/user/applet/editUserInfo",
					method: "POST",
					data: {
